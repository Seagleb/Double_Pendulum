...
// Objects
class Pendulum {
  constructor(anchor_x, anchor_y, arm_len1, start_theta1, arm_len2, start_theta2, mass1, mass2, bob_radius) {
    this.factor = .005
  ...
  update() {
    ...
    this.angular_acceleration1 = ( (-this.gravity * (2 * this.mass1 + this.mass2) * Math.sin(this.theta1)) 
                                  ...
                                  - (2 * Math.sin(this.theta1 - this.theta2) * this.mass2 * (Math.pow(this.angular_velocity2, 2) * this.arm_len2 + Math.pow(this.angular_velocity1, 2) * this.arm_len1 * Math.cos(this.theta1 - this.theta2)))
                                  / (this.arm_len1 * (2 * this.mass1 + this.mass2 - this.mass2 * Math.cos(2 * this.theta1 - 2 * this.theta2))))
    ...
    this.angular_acceleration2 = (2 * Math.sin(this.theta1 - this.theta2) 
                                  * (Math.pow(this.angular_velocity1, 2) * this.arm_len1 * (this.mass1 + this.mass2)
                                  ...
                                  / (this.arm_len2 * ((2 * this.mass1 + this.mass2 - this.mass2 * Math.cos(2 * this.theta1 - 2 * this.theta2)))))
...
let objects
function init() {
  objects = [new Pendulum(canvas.width / 2, canvas.height/2, 100, Math.PI, 100, Math.PI/2, 10, 10, 10)]
